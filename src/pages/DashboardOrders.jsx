  ...
  const fetchAnalytics = async () => {
    ...
    try {
      const response = await fetch(
        "http://localhost:8080/fin/v1/api/analytics/orders"
      );
      if (response.ok) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(`Upload gagal: ${err.error || "Terjadi kesalahan"}`, {
          id: loadingToast,
        });
      }
    } catch (error) {
              ...
              <div className="flex-[2] flex flex-col gap-4 min-h-0">
                ...
                <div className="flex-1 min-h-0 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col">
                  <div className="flex items-center justify-between mb-2 flex-none">
                    ...
                    <button className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-gray-500">
                      <Filter size={16} />
                    </button>
                  </div>
                ...
                {/* Daily Orders Chart */}
                <div className="flex-1 min-h-0 bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col">
                  <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-2 flex-none">
                    Pesanan Harian
                  </h3>
                  <div className="flex-1 min-h-0">
                    <ResponsiveContainer width="100%" height="100%">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                      {stats.top_products.map((product, index) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-xs text-gray-500 dark:text-gray-400 text-right">
                            {product.quantity}
                          </td>
                          <td className="px-4 py-3 whitespace-nowrap text-xs font-bold text-gray-900 dark:text-white text-right">
...
const StatsCard = ({
  ...
  return (
    <div
      className={`p-3 rounded-2xl border shadow-sm hover:shadow-md transition-all relative overflow-hidden ${
        highlight
              ...
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={data.map((v, i) => ({ i, v }))}>
                  <Line
                    ...
                    dataKey="v"
                    stroke={
                      highlight ? "#ffffff" : trendUp ? "#14b8a6" : "#ef4444"
                    }
                    strokeWidth={2}
                    dot={false}
                  />
