...
const DashboardProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch('http://localhost:8080/fin/v1/api/analytics/products');
      if (response.ok) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 flex-none">
        <div>
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
                <Link to="/orders" className="hover:text-orange-600">Pesanan Toko</Link>
                <span>/</span>
                <span className="text-gray-900 dark:text-white font-medium">Detail Produk</span>
            </div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Analisis Produk</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Detail performa penjualan per SKU.</p>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col overflow-hidden">
                ...
                filteredProducts.map((product, index) => (
                    ...
                    <td className="px-6 py-3">
                        <div className="flex items-center">
                            ...
                            <div className="text-sm font-medium text-gray-900 dark:text-white line-clamp-2 max-w-xs" title={product.product_name}>
        ...
        <div className="px-6 py-3 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center text-sm text-gray-500 dark:text-gray-400 flex-none">
            ...
            <div className="flex gap-2">
                <button className="px-3 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50" disabled>Previous</button>
                <button className="px-3 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 disabled:opacity-50" disabled>Next</button>
            </div>
        </div>
      </div>
    </div>
