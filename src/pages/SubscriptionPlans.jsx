...
const SubscriptionPlans = () => {
  ...
  const getButtonState = (plan) => {
    ...
    if (isCurrent) return { text: 'Paket Aktif', disabled: true, type: 'current' };
    if (isPro) return { text: 'Upgrade Sekarang', disabled: false, type: 'upgrade' };
    ...
    <div className="h-full flex flex-col justify-center overflow-hidden p-4">
      <div className="max-w-5xl mx-auto w-full">
        <div className="text-center mb-6 flex-none">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Pilih Paket Langganan</h2>
          <p className="text-gray-500 dark:text-gray-400 max-w-2xl mx-auto text-sm">
            Sesuaikan dengan kebutuhan bisnis Anda. Upgrade kapan saja untuk fitur lebih lengkap.
          </p>
        </div>
            ...
            return (
              <div 
                ...
                className={`relative bg-white dark:bg-gray-800 rounded-xl shadow-sm border-2 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg flex flex-col ${
                  isCurrent ? 'border-green-500 ring-2 ring-green-100 dark:ring-green-900' : 
                  isPro ? 'border-orange-500' : 'border-gray-100 dark:border-gray-700'
                }`}
              >
                {plan.popular && (
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                    <div className={`inline-flex p-2 rounded-lg ${
                      isPro ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400' : 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400'
                    }`}>
                      <Icon size={20} />
                    </div>
                  ...
                  <p className="text-gray-500 dark:text-gray-400 mb-4 text-xs min-h-[32px]">{plan.description}</p>

                  <div className="flex items-baseline mb-4">
                    <span className="text-3xl font-bold text-gray-900 dark:text-white">{plan.price}</span>
                    {plan.period && <span className="text-gray-500 dark:text-gray-400 ml-1 text-xs">{plan.period}</span>}
                  </div>
                ...
                <div className="border-t border-gray-100 dark:border-gray-700 p-5 bg-gray-50 dark:bg-gray-900/50 rounded-b-xl">
                  <ul className="space-y-2">
                    ...
                    {plan.notIncluded.map((feature, idx) => (
                      <li key={idx} className="flex items-center text-gray-400 dark:text-gray-600">
                        <X size={14} className="text-gray-300 dark:text-gray-600 mr-2 flex-shrink-0" />
