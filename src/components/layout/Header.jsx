...
const Header = () => {
  const { user } = useAuth();
  const { platform, setPlatform, store, setStore } = useFilter();
  ...
  const fetchStores = async () => {
    try {
      const response = await fetch('http://localhost:8080/fin/v1/api/stores');
      if (response.ok) {
        const data = await response.json();
        setStores(data);
        // Set default store if available and none selected
        if (data.length > 0 && store === 'all') {
             // Optional: setStore(data[0].ID); 
        }
      }
    } catch (error) {
  ...
  const selectedStoreName = store === 'all' ? 'Semua Toko' : stores.find(s => s.ID === store)?.Name || 'Toko Saya';
  ...
  return (
    <header className="h-16 bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between px-6 sticky top-0 z-40 transition-colors">
      <div className="flex items-center gap-4">
        ...
        <div className="relative group">
          <button className="flex items-center justify-between gap-2 px-3 py-2 w-[260px] bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-700 transition-all shadow-sm hover:shadow-md">
            <div className="flex items-center gap-3">
              <div className={`p-1.5 rounded-lg ${
                platform === 'shopee' ? 'bg-orange-50' :
                platform === 'tiktok-tokopedia' ? 'bg-gray-50' :
                'bg-blue-50'
              }`}>
                {platform === 'shopee' ? <ShopeeLogo className="w-5 h-5" /> : 
                 platform === 'tiktok-tokopedia' ? <TikTokLogo className="w-5 h-5" /> : 
              ...
              <div className="flex flex-col items-start">
                ...
                <span className="text-sm font-semibold text-gray-700 dark:text-gray-200 capitalize leading-tight">
                  {platform === 'all' ? 'Semua Platform' : 
                   platform === 'tiktok-tokopedia' ? 'TikTok Shop x Tokopedia' : 
                   platform}
                </span>
              </div>
            </div>
          ...
          <div className="absolute top-full left-0 w-full pt-2 hidden group-hover:block z-20">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 py-1 animate-fade-in">
              <button onClick={() => toast('Fitur Multi-Platform segera hadir!', { icon: '🚧' })} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-400 dark:text-gray-500 flex items-center justify-between gap-2 cursor-not-allowed opacity-75">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-gray-300 dark:bg-gray-600"></div> Semua Platform
                </div>
                <span className="text-[10px] bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-gray-500">Soon</span>
              </button>
              <button onClick={() => setPlatform('shopee')} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 flex items-center gap-2">
                <ShopeeLogo className="w-5 h-5" /> Shopee
              </button>
              <button onClick={() => setPlatform('tiktok-tokopedia')} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 flex items-center gap-2">
        ...
        <div className="relative group">
          <button className="flex items-center justify-between gap-2 px-3 py-2 w-[260px] bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-700 transition-all shadow-sm hover:shadow-md">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col items-start">
                <span className="text-[10px] text-gray-400 font-medium uppercase tracking-wider">Toko</span>
           ...
           <div className="absolute top-full left-0 w-full pt-2 hidden group-hover:block z-20">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 py-1 animate-fade-in">
              <button onClick={() => setStore('all')} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200">
                Semua Toko
              </button>
              {stores.map((s) => (
                  <button key={s.ID} onClick={() => setStore(s.ID)} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200">
                      {s.Name}
                  </button>
              ))}
            </div>
        ...
        {/* Date Range Picker */}
        {/* Simulating data delay: Max date is 2 days ago */}
        {/* Dynamic minDate based on route (mock logic) */}
        <DateRangePicker 
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-3 pl-4 border-l border-gray-200 dark:border-gray-700 relative group">
          <button className="flex items-center space-x-3 focus:outline-none">
            <div className={`w-9 h-9 rounded-full flex items-center justify-center bg-gray-100 dark:bg-gray-700 border-2 ${
              user?.subscription === 'pro' ? 'border-yellow-500' : 
              user?.role === 'admin' ? 'border-red-500' : 
            ...
            <div className="hidden md:block text-left">
              ...
              <div className="flex items-center gap-1">
                <p className="text-xs text-gray-500 dark:text-gray-400 capitalize">{user?.role === 'admin' ? 'Super Admin' : user?.subscription || 'Starter'}</p>
                {user?.subscription === 'pro' && <span className="w-2 h-2 rounded-full bg-yellow-500"></span>}
              </div>
            </div>
          ...
          <div className="absolute top-full right-0 pt-2 w-48 invisible opacity-0 translate-y-2 group-hover:visible group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-200 ease-in-out delay-200 group-hover:delay-0 z-50">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 py-1">
              ...
              <button onClick={() => window.location.href = '/login'} className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">
                Keluar
              </button>
            </div>
          </div>
