 ...
 * @param {Array} data - Array of data objects (e.g., monthsSkeleton)
 * @param {Array<string>} keys - Keys to calculate growth for (e.g., ['sales', 'orders'])
 * @returns {Array} - Data with appended growth fields (e.g., 'salesGrowth')
 */
export const calculateMoMGrowth = (data, keys) => {
 ...
 * @param {number} sales
 * @param {number} orders
 * @returns {number} Average Basket Size (Rounded)
 */
export const calculateBasketSize = (sales, orders) => {
 ...
 * @param {Array} monthlyData - Array of 12 months
 * @returns {Array} - Array of 4 Quarters with calculated metrics
 */
export const aggregateByQuarter = (monthlyData) => {
  const quarters = [
 ...
 * @param {Array} dailyData - Array of { tanggal, total } objects
 * @returns {Array} - Array of 7 Days with avgOrders
 */
export const aggregateByDayOfWeek = (dailyData) => {
  const days = [
